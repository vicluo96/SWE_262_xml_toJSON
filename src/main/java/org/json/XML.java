...
public class XML {
     ...
     *
     */
    private static boolean parse(XMLTokener x, JSONObject context, String name, XMLParserConfiguration config, String targetPath, String currentPath)
            throws EarlyTermination {
        char c;
        int i;
        JSONObject jsonObject = null;
        String string;
                ...
                if (token instanceof String) {
                    ...
                    for (;;) {
                        ...
                        if (token == null) {
                            ...
                            // Nested element
                            if (parse(x, jsonObject, tagName, config, targetPath, currentPath)) {
                                if (config.getForceList().contains(tagName)) {
                                    // Force the value to be an array
                                    if (jsonObject.length() == 0) {
                                        context.put(tagName, new JSONArray());
    ...
    public static JSONObject toJSONObject(Reader reader, XMLParserConfiguration config) throws JSONException {
        ...
        MyFunction dummy = (a) -> a;
        while (x.more()) {
            x.skipPast("<");
            if(x.more()) {
                parse(x, jo, null, config, dummy::apply);
            }
        }
        return jo;
    }
    ...
    public static JSONObject toJSONObject(Reader reader, JSONPointer path, JSONObject replacement) throws JSONException{
        ...
        while (x.more()) {
            x.skipPast("<");
            if(x.more() && !foundPathFlag) {
                parse(x, jo, null, XMLParserConfiguration.ORIGINAL, currentPath, targetPath, foundPathFlag, replacement);
            }
        }
        return jo;
    }
